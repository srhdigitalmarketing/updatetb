...
<div class="x_panel">
    ...
    <div class="x_content">
        ...
        <?php if(! empty($playerSidebarAd)) : ?>

            <div class="form-group row">
                <label class="control-label col-md-3">Right sidebar:</label>
                <div class="col-md-9">
                    <?= form_textarea( [
                        'name' => "ads[{$playerSidebarAd->id}][ad_code]",
                        'class' => 'form-control mb-3',
                        'placeholder' => 'Enter ad code here',
                        'rows' => 8
                    ] , base64_decode( $playerSidebarAd->ad_code )) ?>

                    <div class="text-right">
                        status:
                        <?= form_dropdown([
                            'name' => "ads[{$playerSidebarAd->id}][status]",
                            'options' => [
                                'active' => 'active',
                                'paused' => 'paused'
                            ],
                            'selected' => $playerSidebarAd->status
                        ]) ?>
                    </div>

                    <?= form_hidden("ads[{$playerSidebarAd->id}][id]", $playerSidebarAd->id) ?>
